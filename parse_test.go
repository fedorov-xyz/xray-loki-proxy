...
func TestParseLog_DatetimeUTCBasicFormat(t *testing.T) {
	tests := []struct {
		name string
		line string
		want string
...
func TestParseLog_FromEndpointSplit(t *testing.T) {
	tests := []struct {
		name string
		line string
		want LogEntry
...
func TestParseLog_ToEndpointSplit(t *testing.T) {
	tests := []struct {
		name string
		line string
		want LogEntry
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := parseLog(tt.line); err == nil {
				t.Fatal("parseLog() error = nil, want rejection")
			}
		})
	}
}
