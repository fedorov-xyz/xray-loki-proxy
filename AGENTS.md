...
Small Go proxy: accepts Loki `logproto` push requests, parses Xray-core access log lines into `LogEntry`, applies skip rules, optionally notifies on torrent tags, appends JSON lines to `OUTPUT_FILE`.

## Layout

| File | Role |
|------|------|
| `main.go` | HTTP handlers (`/loki/api/v1/push`, readiness), regex `xrayLogFormat`, file writer |
| `parse.go` | `LogEntry`, `parseLog` (route arrows `==>` / `->` / `>>` → `-`; optional PTR → `ToAddr`) |
| `skip.go` | destination parsing, domain/IP skip rules |
| `torrent.go` | batched torrent notify |
| `log.go` / `utils.go` | logging + `getEnv` |
| `parse_test.go` | behavior tests for accepted-traffic log shapes |
...
- Do not assert on `ToAddr` unless DNS is mocked — it comes from live reverse lookup.
